    ...
    try {
        const result = await runInTransaction(async (session) => {
            ...
            const audit = createAudit({
                ...
                description: `Issue to ${manager.name}`,
                event_date_label: "Issue Date",
                fields: {
                    item:                    item._id,
                    manager:                 manager._id,
                    quantity:                Number(quantity),
                    price_on_issue_date:     Number(general_price),
    ...
    try {
        const result = await runInTransaction(async (session) => {
            ...
            const audit = createAudit({
                ...
                actor_type: "proprietor",
                actor_id: proprietor_id,
                description: `Issue Hold Items to ${newManager.name}`,
                event_date_label: "Issue Date",
                fields: {
                    item:                        item._id,
                    old_manager:                 manager._id,
                    worker:                      worker._id,
                    new_manager:                 newManager._id,
                    quantity:                    Number(quantity),
                    old_price:                   Number(price),
                    old_underprocessing_value:   Number(underprocessing_value),
                    old_remarks_from_proprietor: remarks_from_proprietor,
                    ...
                    new_underprocessing_value:   Number(new_underprocessing_value),
                },
                event_date: issueDateObj,
                record_date: dateObj,
            });
    ...
    try {
        const result = await runInTransaction(async (session) => {
            ...
            const audit = createAudit({
                ...
                actor_type: "manager",
                actor_id: manager_id,
                description: `Submit to Proprietor, from ${worker.name}`,
...
export const acceptFromManager = async (req, res) => {
    ...
    try {
        const result = await runInTransaction(async (session) => {
            ...
            const event_date_label = action === "accept" ? "Accept Date" : action === "forfeit" ? "Forfeit Date" : "Hold Date";
            const audit = createAudit({
                action_type: "acceptFromManager",
                actor_type: "proprietor",
                actor_id: proprietor_id,
                description: `Accept from ${manager.name} (${action})`,
                event_date_label,
                fields: {
                    item:                          item._id,
                    worker:                        worker._id,
                    ...
                    to_hold:                       !!to_hold,
                    hold_info:                     preparedHoldInfo,
                    deduction_from_proprietor:     Number(deduction),
                    penalty:                       Number(penalty),
                    partial_payment:               Number(partial_payment),
                    final_remarks_from_proprietor: final_remarks,
                    submit_to_proprietor_date:     submitToProprietorDateObj,
                    hold_date:                     actionDateObj,
                },
