...
export const submitToProprietor = async (req, res) => {
    ...
    const { worker_id, design_number, submit_quantity, price, deduction_from_manager, remarks_from_manager, underprocessing_value, remarks_from_proprietor } = req.body;
    ...
    try {
        ...
        if (dbWorkerIndex === -1) {
            ...
            const dbItemIndex = manager.due_backward[dbWorkerIndex].items.findIndex((i) => (i.item.equals(item._id) && i.quantity >= Number(submit_quantity) && i.price === Number(price) && i.deduction_from_manager === Number(deduction_from_manager) && i.remarks_from_manager === remarks_from_manager && i.remarks_from_proprietor === remarks_from_proprietor && i.underprocessing_value === Number(underprocessing_value)));
            if (dbItemIndex === -1) {
                return res.status(404).json({ message: `${submit_quantity} of ${design_number} not due backward at manager: ${manager_id}, made by worker: ${worker_id} with price: ${price}, deduction from manager: ${deduction_from_manager} and remarks from manager: ${remarks_from_manager}, remarks from proprietor: ${remarks_from_proprietor} and underprocessing value: ${underprocessing_value}` });
            }
            else {
        ...
        if (Number(deduction_from_manager) !== 0 || remarks_from_manager !== "" || remarks_from_proprietor !== "") {
            ...
            let sItemIndex = manager.submissions[sWorkerIndex].items.findIndex((i) => (i.item.equals(item._id) && i.price === Number(price) && i.deduction_from_manager === Number(deduction_from_manager) && i.remarks_from_manager === remarks_from_manager && i.remarks_from_proprietor === remarks_from_proprietor && i.underprocessing_value === Number(underprocessing_value)));
            if (sItemIndex === -1) {
                manager.submissions[sWorkerIndex].items.push({ item: item._id, quantity: Number(submit_quantity), price: Number(price), deduction_from_manager: Number(deduction_from_manager), remarks_from_manager: remarks_from_manager, remarks_from_proprietor: remarks_from_proprietor, underprocessing_value: underprocessing_value, date: new Date() });
            }
            else {
...
export const acceptFromManager = async (req, res) => {
    ...
    const { worker_id, design_number, price, deduction_from_manager, remarks_from_manager, underprocessing_value, remarks_from_proprietor, accept_quantity, deduction, final_remarks } = req.body;
    ...
    try {
        ...
        // const [day, month, year] = date.split('/').map(Number);
        // const dateObj = new Date(year, month - 1, day);
        const tdIndex = manager.total_due.findIndex((td) => (td.item.equals(item._id) && td.quantity >= Number(accept_quantity) && td.remarks_from_proprietor === remarks_from_proprietor && td.underprocessing_value === Number(underprocessing_value)));
        if (tdIndex === -1) {
            return res.status(404).json({ message: `${accept_quantity} of ${design_number} with underprocessing value: ${underprocessing_value} and remarks from proprietor: ${remarks_from_proprietor} not issued to manager: ${manager_id}` });
        }
        ...
        const sItemIndex = manager.submissions[sWorkerIndex].items.findIndex((i) => (i.item.equals(item._id) && i.quantity >= Number(accept_quantity) && i.price === Number(price) && i.deduction_from_manager === Number(deduction_from_manager) && i.remarks_from_manager === remarks_from_manager && i.remarks_from_proprietor === remarks_from_proprietor && i.underprocessing_value === Number(underprocessing_value)));
        if (sItemIndex === -1) {
            return res.status(404).json({ message: `${accept_quantity} of ${design_number} not submiited by manager: ${manager_id}, made by worker: ${worker_id} with price: ${price}, deduction from manager: ${deduction_from_manager}, remarks from manager: ${remarks_from_manager}, remarks from proprietor: ${remarks_from_proprietor} and underprocessing value: ${underprocessing_value}` });
        }
        ...
        if (Number(deduction) > (Number(manager.submissions[sWorkerIndex].items[sItemIndex].price) - Number(manager.submissions[sWorkerIndex].items[sItemIndex].deduction_from_manager))) {
            return res.status(400).json({ message: "Deduction can't be more than the price" });
        }

        if (Number(deduction) !== 0 && final_remarks === "") {
            return res.status(400).json({ message: "Final remarks are required if deduction is made" });
        }
        ...
        if (Number(deduction) !== 0 || final_remarks !== "") {
            ...
            const ahItemIndex = manager.accepted_history[ahWorkerIndex].items.findIndex((i) => (i.item.equals(item._id) && i.price === Number(price) && i.deduction_from_manager === Number(deduction_from_manager) && i.remarks_from_manager === remarks_from_manager && i.remarks_from_proprietor === remarks_from_proprietor && i.underprocessing_value === Number(underprocessing_value) && i.deduction_from_proprietor === Number(deduction) && i.final_remarks_from_proprietor === final_remarks));
            if (ahItemIndex === -1) {
                manager.accepted_history[ahWorkerIndex].items.push({ item: item._id, quantity: Number(accept_quantity), price: Number(price), deduction_from_manager: Number(deduction_from_manager), remarks_from_manager: remarks_from_manager, underprocessing_value: Number(underprocessing_value), remarks_from_proprietor: remarks_from_proprietor, deduction_from_proprietor: Number(deduction), final_remarks_from_proprietor: final_remarks, date: new Date() });
            }
            else {
